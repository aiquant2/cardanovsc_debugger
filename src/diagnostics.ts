...
export let diagnosticCollection = vscode.languages.createDiagnosticCollection('haskell');
...
export function parseCabalErrors(
    ...
    const errorRegex = /^\s*(?<file>.+):(?<line>\d+):(?<column>\d+):\s+(?<type>error|warning|info):\s+(?<message>[\s\S]+?)(?=\n\S|$)/gm;
    ...
    while ((match = errorRegex.exec(output)) !== null) {
      ...
      const document = vscodeModule.workspace.textDocuments.find(doc => doc.fileName === filePath);
      if (!document) {
        continue;
      }
      ...
      const range = new vscodeModule.Range(lineNum, colNum, lineNum, colNum + errorLength);
      ...
      if (firstErrorUri === null && severity === vscodeModule.DiagnosticSeverity.Error) {
  ...
  function getDiagnosticSeverity(severity: string, vscodeModule: typeof vscode): vscode.DiagnosticSeverity {
    switch (severity) {
      case "error":
